...
<html>
  ...
  <body>
    <nav class="features">
      <ul class="list">
        <li><h1 style="position: relative; top:10px;left:-4px;">관리페이지</h1></li>
    ...
    <div class="body_wrap">
      <div class="container_wrap" style="width: calc(100% - 16px);margin:0;padding:8px;">
        <form id="apply" action="./apply.php" method="post">
          <input type="hidden" name="secure" value="2J19D08K9">
          <?php
            if (!isset($_GET['opt'])) {
              echo '<div class="opt_name">';
              echo '<h1 style="margin-left:16px;display:inline-block;letter-spacing:2px;">좌석표 설정</h1>';
              echo '<p style="margin-left:48px;display:inline-block">- 복도위치</p>';
              echo '<hr></div>';
              echo '관리관리 핡핡';
            }
            else{
