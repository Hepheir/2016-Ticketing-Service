<?php
  ...
  function disabled_set_table(){
    ...
    for ($rows=1; $rows <= $TABLE_INFO[1]; $rows++) {
      ...
      for ($cols=1; $cols <= $TABLE_INFO[0]; $cols++) {
        if (file_exists($toROOT.'data/config/seat_table/disabled/'.$rows_char.$cols)) {
          echo '<td class="disabled_set" id="disabled"><input type="checkbox" name="disabled_set" value="'.$rows_char.$cols.'" checked=""></td>';
        }
        else {
          echo '<td class="disabled_set" id="not_pinned"><input type="checkbox" name="disabled_set" value="'.$rows_char.$cols.'"></td>';
        }
        if ($cols == $TABLE_INFO[2]) {
    ...
    for ($rows=1; $rows <= $TABLE_INFO[1]; $rows++) {
      ...
      for ($cols=1; $cols <= $TABLE_INFO[0]; $cols++) {
        ...
        elseif (file_exists($toROOT.'data/config/seat_table/part_'.$PART.'/vip\/'.$rows_char.($cols - $cols_shift))) {
          echo '<td class="vip_set" id="vip" style="opacity: 1; cursor: inherit;"><input type="checkbox" name="vip_set" value="'.$rows_char.($cols - $cols_shift).'" checked=""></td>';
        }
        else {
          echo '<td class="vip_set" id="not_pinned"><input type="checkbox" name="vip_set"></td>';
        }
        if ($cols == $TABLE_INFO[2]) {
          echo '<td id="disabled"></td>';
        }
